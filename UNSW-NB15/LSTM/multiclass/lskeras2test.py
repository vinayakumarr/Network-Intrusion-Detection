...
model = Sequential()
model.add(LSTM(16,input_dim=41, return_sequences=True))  # try using a GRU instead, for fun
...
np.savetxt('kddresults/lstm3layer/lstm3predicted.txt', np.transpose([y_test1,y_pred]), fmt='%01d')
'''

# try using different optimizers and different optimizer configs
model.load_weights("kddresults/lstm3layer/checkpoint-87.hdf5")
#y_train1 = y_test
